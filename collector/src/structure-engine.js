...
function minMax(values){let low=Infinity,high=-Infinity,count=0;for(const raw of values){const v=num(raw);if(v===null)continue;if(v<low)low=v;if(v>high)high=v;count++}return count?{low,high}:null}
function ohlc(rows){if(!rows.length)return null;let open=null,close=null,high=-Infinity,low=Infinity,volume=0,count=0;for(const r of rows){const p=num(r.price);if(p===null)continue;if(open===null)open=p;close=p;if(p>high)high=p;if(p<low)low=p;volume+=num(r.notional_usd)||0;count++}if(!count)return null;return{open,high,low,close,volume_usd:volume}}
